...
module.exports.calculateVenuesDailyTotals = (event, context, callback) => {
    ...
    prmSet.forEach(function(prms){
		let customerId = prms ? prms.customerId : null,
		...
		dynamoDb.get({ TableName: dynamoDbTable, Key: { id: customerId } }, (error, data) => { // get customer record
			if (error) {
				...
				//callback(null, util.getCallbackBody(false, 501,  'Customer is not active'));
				return;
			}
			else if (!data.Item.purplePublicKey || !data.Item.purplePrivateKey) {
				//callback(null, util.getCallbackBody(false, 401, 'Customer is missing wifi access keys'));
				return;
			}
			else {
				// these are globals to request
				const publicKey = data.Item.purplePublicKey, 
					privateKey = data.Item.purplePrivateKey, 
        ...
        if (err) {
            ...
            data.forEach(function(customer){
                if (customer.purpleAccountId && customer.purplePublicKey && customer.purplePrivateKey) {
                    ...
                    console.log('triggering aggregation for ' + customer.customerName+ ' from '+msg.from+ ' to '+msg.to);
